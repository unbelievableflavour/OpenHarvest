...
namespace Tests
{
    public class ItemLoaderTest
    {
        ...
        public void ItCanInstantiateEveryItem()
        {
            ...
            foreach (var item in Definitions.ItemsWithInformation)
            {
                if (string.IsNullOrEmpty(item.Value.prefabFileName))
                {
                    continue;
                }

                GameObject spawnedItem = Definitions.InstantiateItem(item.Value.prefabFileName);
            ...
            foreach (HarvestDataTypes.Item item in DatabaseManager.Instance.items.items)
            {
                ...
                GameObject spawnedItem = Definitions.InstantiateItem(item.prefab.transform.name);
                Object.DestroyImmediate(spawnedItem);
            }
        }
        ...
        public void ItHasCorrectRigidBodyOptions()
        {
            ...
            foreach (var item in Definitions.ItemsWithInformation)
            {
                ...
                if(item.Value.prefabFileName == "Wallet"){
                    continue;
                }
        ...
        public void ItCanLoadItemInformationForEveryItem()
        {
            ...
            foreach (var item in Definitions.ItemsWithInformation)
            {
                ...
                if(spawnedItem.GetComponent<ItemInformation>().getItemId() != "")
                {
                    Assert.AreEqual(spawnedItem.GetComponent<ItemInformation>().getItemId(), item.Value.itemId);
                }
            ...
            foreach (HarvestDataTypes.Item item in DatabaseManager.Instance.items.items)
            {
                ...
                GameObject spawnedItem = Definitions.InstantiateItem(item.prefab.transform.name);
                Assert.AreEqual(spawnedItem.GetComponent<ItemInformation>().getItemId(), item.itemId);
                Object.DestroyImmediate(spawnedItem);
            }
        }
