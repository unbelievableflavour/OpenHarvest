...
namespace Tests
{
    public class PigPlateauTest
    {
        const string PrefabPath = "Assets/Components/PlaceableObjects/PlateauPig/PlateauPig.prefab";
        ...
        public void ItChecksIfAllRequiredFieldsAreNotEmpty()
        {
            ...
            if (prefab == null)
            {
                Assert.Ignore("PigPlateau prefab not yet created — rebuild it in the editor with AnimalInformation.");
                return;
            }

            var instance = Object.Instantiate(prefab);
            var info = instance.GetComponentInChildren<AnimalInformation>(true);
            Assert.IsNotNull(info, "PlateauPig is missing AnimalInformation.");
