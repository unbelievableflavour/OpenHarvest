...
namespace BNG
{
    public class PlayerRotationCustom : MonoBehaviour
    {
        ...
        [Header("Screen Fade")]
        [Tooltip("Use ScreenFader on rotation if true.")]
        public bool fadeScreenOnRotation = true;

        [Tooltip("If FadeScreenOnRotation = true, fade the screen at this speed.")]
        public float rotationFadeSpeed = 10f;
        ...
        void setupVariables()
        {
            var playerController = GetComponent<BNGPlayerController>();
            var cameraRig = playerController.CameraRig;
        ...
        public void DoFadeRotation(float xInput)
        {
            StartCoroutine(DoFadeRotationNumerator(xInput));
        }

        public virtual IEnumerator DoFadeRotationNumerator(float xInput)
        {
            // Reset rotation amount before retrieving inputs
            rotationAmount = 0;
            ...
            if (xInput >= 0.1f && previousXInput < 0.1f)
            {
                if (fadeScreenOnRotation && fader)
                {
                    fader.FadeInSpeed = rotationFadeSpeed;
                    fader.DoFadeIn();
                    if (rotationFadeSpeed > 0)
                    {
                        yield return new WaitForSeconds(rotationFadeSpeed * 0.02f);
                        fader.DoFadeOut();
                    }
                }
