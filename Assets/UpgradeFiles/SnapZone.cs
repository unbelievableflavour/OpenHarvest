...
namespace BNG {
    public class SnapZone : MonoBehaviour {
        ...
        Transform mainCam;
        private int maxUseableDistance = 5;
        ...
        public void GrabGrabbable(Grabbable grab) {
            //START_CUSTOM
            if (grab.GetComponent<ItemStack>()) {
                var uniqueItemId = grab.GetComponent<ItemStack>().GetUniqueId();
                //If this doesn't work. Maybe cast LastDropTime & timeLastItemDropped to int
                if (uniqueItemId == GameState.Instance.idLastItemDropped && grab.LastDropTime == GameState.Instance.timeLastItemDropped)
                {
                    //Grabbing same item (currently only with stack). Drop the item and dont snap it at all.
                    return;
                }
                GameState.Instance.idLastItemDropped = uniqueItemId;
                GameState.Instance.timeLastItemDropped = grab.LastDropTime;
            }

            var stackableZone = GetComponent<StackableSnapZone>();
            if (HeldItem != null && stackableZone != null && !stackableZone.itemIsStackable(grab))
            {
                return;
            }
            ...
            if(HeldItem != null) {
                if (stackableZone != null) {
                    stackableZone.AttachItem(grab);
                    return;
                }
                else {
                    ReleaseAll();
        ...
        public void ReleaseAll() {
            ...
            if(HeldItem != null) {
                ...
                if (stackableZone)
                {
                    stackableZone.DetachItem(HeldItem);
                    return;
                }
            }
